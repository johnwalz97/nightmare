...
function Nightmare(options) {
  ...
  if(options.loadTimeout && options.gotoTimeout && options.loadTimeout < options.gotoTimeout){
    console.log(`WARNING:  load timeout of ${options.loadTimeout} is shorter than goto timeout of ${options.gotoTimeout}`);
  }
  ...
  console.log('queuing process start');
  this.queue((done) => {
  ...
  Object.keys(Nightmare.childActions).forEach(function(key){
    console.log('queueing child action addition for "%s"', key);
...
function handleExit(code, instance, cb){
  ...
  console.log('electron child process exited with code ' + code + ': ' + help[code]);
...
function endInstance(instance, cb, forceKill) {
  ...
  if (instance.proc && instance.proc.connected) {
    ...
    console.log('electron child process not started yet, skipping kill.');
    cb();
  }
}
...
Nightmare.prototype.goto = function(url, headers) {
  console.log('queueing action "goto" for %s', url);
  var self = this;
...
function queued (name, fn) {
  return function action () {
    console.log('queueing action "' + name + '"');
