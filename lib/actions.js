...
exports.visible = function(selector, done) {
  console.log('.visible() for ' + selector);
...
exports.exists = function(selector, done) {
  console.log('.exists() for ' + selector);
  this.evaluate_now(function(selector) {
...
exports.mousedown = function(selector, done) {
  console.log('.mousedown() on ' + selector);
  this.evaluate_now(function (selector) {
    var element = document.querySelector(selector);
    if (!element) {
      throw new Error('Unable to find element by selector: ' + selector);
...
exports.mouseup = function(selector, done) {
  console.log('.mouseup() on ' + selector);
  this.evaluate_now(function (selector) {
    var element = document.querySelector(selector);
    if (!element) {
      throw new Error('Unable to find element by selector: ' + selector);
...
exports.mouseover = function(selector, done) {
  console.log('.mouseover() on ' + selector);
  this.evaluate_now(function (selector) {
    var element = document.querySelector(selector);
    if (!element) {
      throw new Error('Unable to find element by selector: ' + selector);
...
exports.type = function() {
  ...
  console.log('.type() %s into %s', text, selector);
  var self = this;

  focusSelector.bind(this)(function(err) {
    if(err) {
      console.log('Unable to .type() into non-existent selector %s', selector);
      return done(err);
    }
...
exports.insert = function(selector, text, done) {
  ...
  console.log('.insert() %s into %s', text, selector);
  var child = this.child;

  focusSelector.bind(this)(function(err) {
    if(err) {
      console.log('Unable to .insert() into non-existent selector %s', selector);
      return done(err);
    }
...
exports.wait = function () {
  ...
  if (args.length < 2) {
    console.log('Not enough arguments for .wait()');
    return done();
  }

  var arg = args[0];
  if (typeof arg === 'number') {
    console.log('.wait() for ' + arg + 'ms');
    ...
    console.log('.wait() for '+arg+' element'+(timeout ? ' or '+timeout+'msec' : ''));
    waitelem.apply({ timeout: timeout }, [this, arg, done]);
  }
  else if (typeof arg === 'function') {
    console.log('.wait() for fn');
    args.unshift(this);
...
exports.inject = function (type, file, done) {
  ...
  if (type === 'js') {
    ...
    console.log('unsupported file type in .inject()');
    done();
  }
};
...
exports.useragent = function(useragent, done) {
  console.log('.useragent() to ' + useragent);
...
exports.pdf = function (path, options, done) {
  ...
  this.child.call('pdf', path, options, function (error, pdf) {
    ...
    console.log('.pdf() captured with length %s', buf.length);
    path ? fs.writeFile(path, buf, done) : done(null, buf);
  });
};
